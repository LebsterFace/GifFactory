...
const roundRect = (x: number, y: number, width: number, height: number, radiusParam = 5) => {
	ctx.beginPath();
	ctx.moveTo(x + radiusParam, y);
	ctx.lineTo(x + width - radiusParam, y);
	ctx.quadraticCurveTo(x + width, y, x + width, y + radiusParam);
	ctx.lineTo(x + width, y + height - radiusParam);
	ctx.quadraticCurveTo(x + width, y + height, x + width - radiusParam, y + height);
	ctx.lineTo(x + radiusParam, y + height);
	ctx.quadraticCurveTo(x, y + height, x, y + height - radiusParam);
	ctx.lineTo(x, y + radiusParam);
	ctx.quadraticCurveTo(x, y, x + radiusParam, y);
	ctx.closePath();
};
...
roundRect(MARGIN, MARGIN, WIDTH, HEIGHT, 20);
